...
class AssetControllerE2ETest {
    ...
    @Container
    static GenericContainer<?> nats = new GenericContainer<>(DockerImageName.parse("nats:2.10-alpine"))
            .withExposedPorts(4222)
            .waitingFor(Wait.forLogMessage(".*Listening for client connections.*\\n", 1));

    @Container
    static GenericContainer<?> clickhouse = new GenericContainer<>(DockerImageName.parse("clickhouse/clickhouse-server:24.3-alpine"))
    ...
    static void dynamicProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("storage.minio.publicUrl", () -> "http://" + minio.getHost() + ":" + minio.getMappedPort(9000)); // We use dynamic port for client test too
    ...
    private String getAuthToken() {
        ...
        String body = "{ \"email\": \"jedi-" + UUID.randomUUID() + "@order.com\", \"password\": \"force\" }";
    ...
    static void setupMinioBucket() throws Exception {
        ...
        org.testcontainers.containers.Container.ExecResult result = minio.execInContainer(
                "sh", "-c", 
                "mkdir -p /data/deathstar-assets && curl -X PUT http://127.0.0.1:9000/deathstar-assets/"
        );
        // Note: minio client native way might be needed if curl fails but usually minio creates buckets if folder exists inside /data
    }
    ...
    void e2eUploadFlow_TokenTamperingAndHappyPath() throws Exception {
        ...
        // The URL is double-encoded due to JSON serialization or Spring Test RestTemplate internals (%253B instead of %3B).
        ...
        String tamperedUrl = uploadUrlStr.replaceFirst("X-Amz-Signature=[0-9a-f]+", "X-Amz-Signature=1");
        if (tamperedUrl.equals(uploadUrlStr)) {
            tamperedUrl = uploadUrlStr.replaceFirst("x-amz-signature=[0-9a-f]+", "x-amz-signature=1");
        }
        
        HttpHeaders uploadHeaders = new HttpHeaders();
        uploadHeaders.setContentType(MediaType.valueOf("text/csv"));
        HttpEntity<byte[]> uploadEntity = new HttpEntity<>("A,B,C\n1,2,3".getBytes(java.nio.charset.StandardCharsets.UTF_8), uploadHeaders);
        
        ResponseEntity<String> tamperResponse = restTemplate.exchange(
                new java.net.URI(tamperedUrl),
                HttpMethod.PUT,
                uploadEntity,
                String.class
        );
