...
@Testcontainers
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, classes = VaderApplication.class)
@ActiveProfiles("e2e")
class TaskFieldDefinitionControllerE2ETest {
    ...
    static GenericContainer<?> clickhouse =
            new GenericContainer<>(DockerImageName.parse("clickhouse/clickhouse-server:24.3-alpine"))
    ...
    static void dynamicProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("audit.clickhouse.jdbc-url", () -> "jdbc:ch:http://" + clickhouse.getHost() + ":" + clickhouse.getMappedPort(8123) + "/default");
    ...
    private String registerAndGetToken(String emailPrefix) {
        ...
        String body = "{ \"email\": \"" + emailPrefix + "-" + UUID.randomUUID() + "@order.com\", \"password\": \"force\" }";
        ResponseEntity<String> response = restTemplate.postForEntity("/auth/register", new HttpEntity<>(body, headers), String.class);
    ...
    void testTenantIsolationForCustomFields() {
        ...
        TaskFieldDefinitionRequest reqA = new TaskFieldDefinitionRequest()
                .name("Story Points")
                .fieldType(FieldType.INTEGER)
                .bucketType(BucketType.STATIC);

        ResponseEntity<TaskFieldDefinition> createResA = restTemplate.postForEntity(
                "/tasks/fields", new HttpEntity<>(reqA, headersA), TaskFieldDefinition.class);
        ...
        TaskFieldDefinitionRequest reqB = new TaskFieldDefinitionRequest()
                .name("Client Name")
                .fieldType(FieldType.STRING)
                .bucketType(BucketType.DYNAMIC);

        ResponseEntity<TaskFieldDefinition> createResB = restTemplate.postForEntity(
                "/tasks/fields", new HttpEntity<>(reqB, headersB), TaskFieldDefinition.class);
        
        assertThat(createResB.getStatusCode()).isEqualTo(HttpStatus.CREATED);

        // 4. Verify Isolation
        ResponseEntity<List<TaskFieldDefinition>> getResA = restTemplate.exchange(
                "/tasks/fields", HttpMethod.GET, new HttpEntity<>(headersA), new ParameterizedTypeReference<>() {});
        
        ResponseEntity<List<TaskFieldDefinition>> getResB = restTemplate.exchange(
                "/tasks/fields", HttpMethod.GET, new HttpEntity<>(headersB), new ParameterizedTypeReference<>() {});
        ...
        boolean hasStoryPointsInA = fieldsA.stream().anyMatch(f -> "Story Points".equals(f.getName()));
        boolean hasClientNameInA = fieldsA.stream().anyMatch(f -> "Client Name".equals(f.getName()));
        
        boolean hasClientNameInB = fieldsB.stream().anyMatch(f -> "Client Name".equals(f.getName()));
        boolean hasStoryPointsInB = fieldsB.stream().anyMatch(f -> "Story Points".equals(f.getName()));
