...
public class BoardTaskService {
    ...
    @Transactional
    public com.deathstar.vader.dto.generated.BoardColumn createColumn(UUID boardId, String title, Integer orderIndex) {
    ...
    public void updateTask(
        ...
        if (title != null) updates.put(UUID.fromString("00000000-0000-0000-0000-000000000000"), title);
    ...
    public List<com.deathstar.vader.dto.generated.Board> getBoards() {
        List<Item> items = itemQueryService.getItemsByDynamicProperty(FieldConstants.TYPE_ID, "Board");
        return items.stream()
                ...
                .map(item -> {
                    ...
                    if (staticAttrs.containsKey(FieldConstants.CREATED_AT_ID)) {
                        board.setCreatedAt(java.time.OffsetDateTime.parse(staticAttrs.get(FieldConstants.CREATED_AT_ID).toString()));
                    } else {
    ...
    public com.deathstar.vader.dto.generated.Board getBoardWithColumns(UUID boardId) {
        ...
        if (boardItem.staticAttributes().containsKey(FieldConstants.CREATED_AT_ID)) {
            board.setCreatedAt(java.time.OffsetDateTime.parse(boardItem.staticAttributes().get(FieldConstants.CREATED_AT_ID).toString()));
        } else {
            board.setCreatedAt(java.time.OffsetDateTime.now());
        }
        
        List<Item> colItems = itemQueryService.getItemsByDynamicProperty(FieldConstants.BOARD_ID, boardId.toString());
        List<com.deathstar.vader.dto.generated.BoardColumn> columns = colItems.stream()
                .filter(item -> "BoardColumn".equals(item.dynamicAttributes().get(FieldConstants.TYPE_ID)))
                .map(item -> {
                    var col = new com.deathstar.vader.dto.generated.BoardColumn();
                    col.setId(item.itemId());
                    col.setBoardId(boardId);
                    if (item.staticAttributes().containsKey(FieldConstants.TITLE_ID)) {
                        col.setTitle(item.staticAttributes().get(FieldConstants.TITLE_ID).toString());
                    }
                    if (item.dynamicAttributes().containsKey(FieldConstants.ORDER_INDEX_ID)) {
                        col.setOrderIndex(Integer.parseInt(item.dynamicAttributes().get(FieldConstants.ORDER_INDEX_ID).toString()));
                    }
                    if (item.staticAttributes().containsKey(FieldConstants.CREATED_AT_ID)) {
                        col.setCreatedAt(java.time.OffsetDateTime.parse(item.staticAttributes().get(FieldConstants.CREATED_AT_ID).toString()));
                    } else {
                ...
                .sorted(java.util.Comparator.comparing(
                        com.deathstar.vader.dto.generated.BoardColumn::getOrderIndex,
    ...
    public List<com.deathstar.vader.dto.generated.BoardTask> getTasksForBoard(UUID boardId) {
        ...
        List<String> columnIds = board.getColumns() != null ? 
            board.getColumns().stream().map(c -> c.getId().toString()).toList() : List.of();

        if (columnIds.isEmpty()) return List.of();

        List<Item> items = itemQueryService.getItemsByDynamicPropertyIn(FieldConstants.STATUS_ID, columnIds);
        ...
        return items.stream().map(item -> {
            ...
            if (staticAttrs.containsKey(FieldConstants.DESCRIPTION_ID)) {
                task.setDescription(staticAttrs.get(FieldConstants.DESCRIPTION_ID).toString());
            }
            if (staticAttrs.containsKey(FieldConstants.PRIORITY_ID)) {
                task.setPriority(staticAttrs.get(FieldConstants.PRIORITY_ID).toString());
            }
            if (staticAttrs.containsKey(FieldConstants.DUE_DATE_ID)) {
                task.setDueDate(java.time.LocalDate.parse(staticAttrs.get(FieldConstants.DUE_DATE_ID).toString()));
            }

            if (dynamicAttrs.containsKey(FieldConstants.STATUS_ID)) {
                task.setStatus(UUID.fromString(dynamicAttrs.get(FieldConstants.STATUS_ID).toString()));
            }
            if (dynamicAttrs.containsKey(FieldConstants.LEXRANK_ID)) {
                task.setLexRank(dynamicAttrs.get(FieldConstants.LEXRANK_ID).toString());
            }

            java.util.Map<String, String> customAttributes = new java.util.HashMap<>();
            for (java.util.Map.Entry<UUID, Object> entry : dynamicAttrs.entrySet()) {
                if (!entry.getKey().equals(FieldConstants.STATUS_ID) && !entry.getKey().equals(FieldConstants.LEXRANK_ID)) {
                    customAttributes.put(entry.getKey().toString(), entry.getValue().toString());
                }
            }
